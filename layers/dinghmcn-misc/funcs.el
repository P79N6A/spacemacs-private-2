...
(defun dinghmcn/insert-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ";")))

(defun dinghmcn/delete-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ";")
...
(defun dinghmcn/insert-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ",")))

(defun dinghmcn/delete-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ",")
...
(defun dinghmcn/load-my-layout ()
  (interactive)
  (persp-load-state-from-file (concat persp-save-dir "dinghm")))
...
(defun dinghmcn/tab-region (N)
  (interactive "p")
  (if (use-region-p)
      (dinghmcn/indent-region 4)               ; region was selected, call indent-region
    (insert "    ")                   ; else insert four spaces as expected
    ))

(defun dinghmcn/untab-region (N)
  (interactive "p")
  (dinghmcn/indent-region -4))

(defun dinghmcn/hack-tab-key ()
  (interactive)
  (local-set-key (kbd "<tab>") 'dinghmcn/tab-region)
  (local-set-key (kbd "<S-tab>") 'dinghmcn/untab-region)
  )

;; I'm don't like this settings too much.
;; (add-hook 'prog-mode-hook 'dinghmcn/hack-tab-key)
...
org-files and bookmarks"
  ...
  (helm :buffer "*helm: utities*"
        :sources `(,(dinghmcn//hotspots-sources))))

(defun dinghmcn//hotspots-sources ()
...
(defun dinghmcn/browser-refresh--chrome-applescript ()
  (interactive)
  (do-applescript
   (format
    "
...
(defun dinghmcn/hidden-dos-eol ()
  "Do not show ^M in files containing mixed UNIX and DOS line endings."
  (interactive)
  (setq buffer-display-table (make-display-table))
  (aset buffer-display-table ?\^M []))

(defun dinghmcn/remove-dos-eol ()
  "Replace DOS eolns CR LF with Unix eolns CR"
  (interactive)
  (goto-char (point-min))
  (while (search-forward "\r" nil t) (replace-match "")))

(defun dinghmcn/insert-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
  (insert (dinghmcn/retrieve-chrome-current-tab-url)))

(defun dinghmcn/retrieve-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
...
(defun dinghmcn/single-lines-only ()
...
(defun dinghmcn/named-term (name)
  (interactive "sName: ")
  (ansi-term "/bin/zsh" name))


(defun dinghmcn/ash-term-hooks ()
...
(defun dinghmcn/word-count-for-chinese ()
  "「較精確地」統計中/日/英文字數。
- 文章中的註解不算在字數內。
...
(defun dinghmcn/evil-quick-replace (beg end )
  (interactive "r")
...
(defun dinghmcn/git-project-root ()
...
(defun dinghmcn/iterm-shell-command (command &optional prefix)
  "cd to `default-directory' then run COMMAND in iTerm.
With PREFIX, cd to project root."
  (interactive (list (read-shell-command
                      "iTerm Shell Command: ")
                     current-prefix-arg))
  (let* ((dir (if prefix (dinghmcn/git-project-root)
                default-directory))
...
(defun dinghmcn/my-mc-mark-next-like-this ()
  (interactive)
  (if (region-active-p)
...
(defun my-open-file-in-external-app (file)
  ...
  (interactive)
  (let ((default-directory (dinghmcn/git-project-root))
        (file-path file))
    (if file-path
        (cond
...
(defun dinghm/github-browse-commit ()
